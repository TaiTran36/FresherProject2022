...
<div class="container">
    @for ($i = 0; $i < count($posts); $i=$i+3) <div class="row">
        @for ($j = 0; $j < 3; $j++) <div class="col">
                ...
                <div class="content">
                    <div class="post-meta mb-1">
                        @for($k = 0; $k < count($posts[$i + $j]->category); $k++)
                            ...
                            <a href="#" class="category fw-bold">{{$posts[$i + $j]->category[$k]}}</a>
                            @else
                            <a href="#" class="category fw-bold">{{$posts[$i + $j]->category[$k]}}</a>,
                            @endif
                        @endfor
                            &mdash;
                            <span class="date">{{Carbon\Carbon::parse($posts[$i + $j]->created_at)->format('F d, y')}}</span>
                    </div>
                    <h2 class="heading mb-3 fw-bold">
                        <a href="{{ route('post.read', $posts[$i + $j]->url) }}">{{$posts[$i + $j]->title}}</a>
                    </h2>
