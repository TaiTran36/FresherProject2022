                        ...
                        <div class="form-check">
                            @foreach ($post_categories as $post_category)
                                <input name="categories[]" class="form-check-input" type="checkbox"
                                    value="{{ $category->id }}"
                                    {{ in_array($category->name, $post_category->pluck('category')->toArray()) ? 'checked' : '' }}
                                    id="flexCheckDefault">
            ...
            <div class="form-group">
                ...
                <div>
                    ...
                    <input type="file" class="form-control" id="image" onchange="getFileData(this);" name="image"
                        placeholder="image" value="{{ $getpostById[0]->photo_path }}" />
                    <input type="text" name="image_old" placeholder="image" value="{{ $getpostById[0]->photo_path }}"
                        hidden />
                </div>
            </div>
            <div class="form-group">
                <label for="content">Content</label>
                <textarea rows="10" type="text" class="form-control" id="name" name="content" placeholder="Enter content"
                    value="{{ $getpostById[0]->content }}" required>{{ $getpostById[0]->content }}</textarea>
            </div>
            <input type="text" name="id" placeholder="id" value="{{ $getpostById[0]->id }}" hidden />
            <center><button id="submit_edit" type="submit" class="btn btn-primary">Save</button></center>
        </form>
    </div>
