                ...
                <div class="top w-11/12 lg:w-7/12 mx-auto my-14">
                    <div class="info mx-auto">
                        <a href="/author/{{ $post->writer_username_login }}/posts">
                            <div class="personpic w-16 mx-auto"><img
                                    href="/author/{{ $post->writer_username_login }}/posts" class="rounded-full w-full"
                                    onerror="this.src='/profile/error_img/not_found.png'"
                                    src="{{ asset('/profile/' . $post->writer_avatar) }}"></div>
                            <h1 class="text-center text-gray-800 text-lg text-3xl">{{ $post->writer_name }}</h1>
                        </a>
                        <h3 class="text-center text-gray-500">{{ $post->created_at }}</h3>
                    </div>
                    <h1 class="px-6 text-5xl lg:text-5xl text-center tracking-wide leading-normal mt-6"><b>
                            {{ $post->title }}</b></h1>
                    <br>
                    ...
                    <div class="wrapper">
                        ...
                        <div class="radio_group">
                            <input @guest disabled @endguest id="likebtn" type="radio" data-waschecked="true"
                                name="like" value="1" @if ($liked == 1) checked @endif>
                            <label for="like">
                        ...
                        <div class="radio_group">
                            <input @guest disabled @endguest id="dislikebtn" type="radio" data-waschecked="true"
                                name="like" value="-1" @if ($liked == -1) checked @endif>
                            <label for="like">
                        ...
                        <a id="count_dislike"
                            class="text-gray-500 text-lg font-bold mt-10 mb-6">{{ $count_dislike }}</a>
                    </div>
                    @guest
                        ...
                        <div class="card mt-4">
                            ...
                            <div class="card-body">
                                ...
                                <form class="add-comment mb-3">
                                    ...
                                    <textarea maxlength="500" style="width:93%" name="comment" id="comment"
                                        class=" my-2 w-full w-2/3 mr-3 p-3 rounded-md border border-gray-300" @guest
                                        placeholder="You must login before sending comment !" disabled @else placeholder="Enter your comment..." @endguest></textarea>

                                    <button type="button" id="save-comment" data-post="{{ $post->id }}" @guest
                                        style="background-color: rgb(154, 154, 154)" @endguest
